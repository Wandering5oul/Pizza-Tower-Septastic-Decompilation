var _cam_x, _cam_y, lay_arr, i, lay, lay_name, per, xof, yof, per_x, per_y, bg_x, bg_y;

_cam_x = camera_get_view_x(view_camera[0]);
_cam_y = camera_get_view_y(view_camera[0]);
lay_arr = layer_get_all();

for (i = 0; i < array_length(lay_arr); i++)
{
    lay = lay_arr[i];
    lay_name = layer_get_name(lay);
    ...
    switch (lay_name)
    {
        ...
        case "Backgrounds_still2":
            ...
            per_x = _cam_x / (room_width - 960);
            per_y = _cam_y / (room_height - 540);
            bg_x = calculate_parrallax_still_x(lay, per);
            bg_y = calculate_parrallax_still_y(lay, per);

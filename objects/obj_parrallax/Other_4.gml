var asset_layers, asset_parallax, i, b, p, q, j, _asset, _x, _y, spr;
...
asset_layers = ["Assets_BG", "Assets_BG1", "Assets_BG2", "Assets_stillBG1", "Assets_FG", "Assets_FG1", "Assets_FG2"];
asset_parallax = [[0.1, 0.1], [0.05, 0.05], [0.1, 0.1], [0.05, 0], [-0.1, -0.1], [-0.05, -0.05], [-0.1, -0.1]];
...
for (i = 0; i < array_length(asset_layers); i++)
{
    ...
    {
        b = layer_get_id(b);
        q = layer_get_all_elements(b);
        
        for (j = 0; j < array_length(q); j++)
        {
            _asset = q[j];
            _x = layer_sprite_get_x(_asset);
            _y = layer_sprite_get_y(_asset);
            spr = layer_sprite_get_sprite(_asset);
            
            if (p[0] != 0)
                layer_sprite_x(_asset, (_x - (_x * p[0])) + (240 * p[0]));
            
            if (p[1] != 0 && spr != spr_industrialpipe && spr != bg_farmdirtwall)
                layer_sprite_y(_asset, (_y - (_y * p[1])) + (135 * p[1]));
        }
    }
}
...
bg_ground1yoffset = layer_get_y("Backgrounds_Ground1");
bg_ground2yoffset = layer_get_y("Backgrounds_Ground2");
bg_ground3yoffset = layer_get_y("Backgrounds_Ground3");

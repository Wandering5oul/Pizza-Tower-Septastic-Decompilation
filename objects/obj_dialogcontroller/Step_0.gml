...
else
{
    ...
    {
        displaytext = string_insert(string_char_at(currenttext, currentpos), displaytext, string_length_scribble(displaytext) + 1);
        ...
        if (!talking)
            ...
            scr_soundeffect(playerdialogsnd[irandom(array_length(playerdialogsnd) - 1)]);
    }
...
{
    while (currentpos <= string_length_scribble(currenttext))
    {
        currentpos++;

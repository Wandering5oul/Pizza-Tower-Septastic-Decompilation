...
{
    var helpFunction, metadata, output, i, argName, desc, hiddenCount, functionName, hidden, terminator, paddingWidth, spaceCount;
    
    if (array_length(argument0) > 1)
    {
        helpFunction = argument0[1];
        
        if (array_contains(allFunctions, helpFunction))
        {
            if (variable_struct_exists(functionData, helpFunction))
            {
                metadata = variable_struct_get(functionData, helpFunction);
                output = helpFunction;
                
                if (variable_struct_exists(metadata, "arguments"))
                {
                    for (i = 0; i < array_length(metadata.arguments); i++)
                ...
                if (variable_struct_exists(metadata, "argumentDescriptions"))
                {
                    ...
                    {
                        argName = metadata.arguments[i];
                        desc = metadata.argumentDescriptions[i];
                        output += (argName + " - " + desc + "\n");
                    }
                }
    ...
    else
    {
        ...
        {
            functionName = availableFunctions[i];
            hidden = false;
            ...
            if (!hidden)
            {
                ...
                {
                    paddingWidth = (width / 2) - (anchorMargin + string_width(functionName));
                    spaceCount = paddingWidth / string_width(" ");
...
variable_global_set("sh_clear", function(argument0)
{
    ...
    else
    {
        ...
        _newLinesCount = floor(visibleHeight / string_height(prompt));

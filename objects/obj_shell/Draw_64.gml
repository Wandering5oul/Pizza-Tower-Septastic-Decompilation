var emWidth, emHeight, promptXOffset, i, newMaxScroll, yOffset, outputStr, cursorPosX, ff, data, spaceCount, suggestion, args, x1, y1, x2, y2, containerHeight, scrollProgress, scrollbarHeight, scrollbarPosition, suggestionsAmount, stringParts, suggestXOffset, scrollbarTotalHeight, scrollbarProgress, yScroll1, yScroll2, y1Col, y2Col;
...
if (isOpen)
{
    ...
    promptXOffset = consolePaddingH + string_width(prompt) + anchorMargin;
    outputHeight = 0;
    ...
    newMaxScroll = max(0, (outputHeight - visibleHeight) + emHeight);
    ...
    cursorPosX = shellOriginX + promptXOffset + string_width(string_copy(consoleString + " ", 1, cursorPos - 1));
    ...
    {
        ff = (array_length(filteredSuggestions) > 0 && array_length(inputArray) == 1) ? filteredSuggestions[suggestionIndex] : inputArray[0];
        data = variable_struct_get(functionData, ff);
        spaceCount = array_length(inputArray) - 1;
        suggestion = (spaceCount == 0) ? ff : "";
    ...
    if (outputHeight > (visibleHeight - emHeight))
    {
        x1 = (shellOriginX + width) - consolePaddingH - scrollbarWidth;
        y1 = shellOriginY + consolePaddingV + 1;
        x2 = x1 + scrollbarWidth;
        y2 = (shellOriginY + height) - consolePaddingV;
        draw_set_color(fontColorSecondary);
        draw_rectangle(x1, y1, x2, y2, false);
        containerHeight = y2 - y1;
        scrollProgress = scrollPosition / ((outputHeight - visibleHeight) + emHeight);
        scrollbarHeight = (visibleHeight / (outputHeight + emHeight)) * containerHeight;
        scrollbarPosition = (containerHeight - scrollbarHeight) * scrollProgress;
    ...
    if (array_length(filteredSuggestions) > 0)
    {
        if (enableAutocomplete && autocompleteMaxLines > 0)
        {
            ...
            suggestionsAmount = min(autocompleteMaxLines, array_length(filteredSuggestions));
            stringParts = _input_string_split(consoleString);
            suggestXOffset = 0;
            ...
            if (suggestionsAmount < array_length(filteredSuggestions))
            {
                ...
                scrollbarTotalHeight = y2 - y1;
                scrollbarHeight = (suggestionsAmount / array_length(filteredSuggestions)) * scrollbarTotalHeight;
                scrollbarProgress = (array_length(filteredSuggestions) - autocompleteScrollPosition) / array_length(filteredSuggestions);
                yScroll1 = y1 + (scrollbarTotalHeight * (1 - scrollbarProgress)) + 1;
                yScroll2 = (yScroll1 + scrollbarHeight) - 1;
                draw_set_color(fontColor);
            ...
            for (i = 0; i < array_length(filteredSuggestions); i++)
            {
                if (i < suggestionsAmount)
                {
                    y1Col = y1 + (i * emHeight);
                    y2Col = ((y1 + (i * emHeight) + emHeight) - 1) + autocompletePadding;

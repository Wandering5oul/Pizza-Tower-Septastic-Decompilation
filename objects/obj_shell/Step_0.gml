var prevConsoleString, leftSide, rightSide, precedingSpaceIndex, i, nextSpaceIndex, suggestion, consoleWords, currentWordLength, args, metadata, deferred, uncompleted, t, x1, y1, x2, y2, lerpValue, instAtCursor;
...
if (!isOpen)
{
    if (_key_combo_pressed(openModifiers, openKey))
        open();
}
else
{
    prevConsoleString = consoleString;
    ...
    else if (_key_combo_pressed([metaKey], 65) || keyboard_check_pressed(vk_home))
    {
        cursorPos = 1;
        targetScrollPosition = maxScrollPosition;
    }
    else if (_key_combo_pressed([metaKey], 69) || keyboard_check_pressed(vk_end))
    {
        cursorPos = string_length(consoleString) + 1;
        targetScrollPosition = maxScrollPosition;
    }
    else if (_key_combo_pressed([metaKey], 75))
    {
        leftSide = string_copy(consoleString, 0, cursorPos - 1);
        rightSide = string_copy(consoleString, cursorPos, (string_length(consoleString) - cursorPos) + 1);
        killedString = rightSide;
        consoleString = leftSide;
        cursorPos = string_length(consoleString) + 1;
        targetScrollPosition = maxScrollPosition;
    }
    else if (_key_combo_pressed([metaKey], 89))
    {
    ...
    else if (_key_combo_pressed([metaKey], 67))
    {
    ...
    else if (_key_combo_pressed([metaKey], 8) || (metaKey == 17 && ord(keyboard_string) == 127))
    {
    ...
    else if (_key_combo_pressed([metaKey], 37))
    {
        precedingSpaceIndex = 1;
        i = cursorPos - 2;
    ...
    else if (_key_combo_pressed([metaKey], 39))
    {
        nextSpaceIndex = string_length(consoleString) + 1;
        i = cursorPos + 2;
    ...
    else if (_keyboard_check_delay(8))
    {
        if (!metaDeleted)
        {
    ...
    else if (_keyboard_check_delay(46))
    {
        consoleString = string_delete(consoleString, cursorPos, 1);
        targetScrollPosition = maxScrollPosition;
    }
    else if (_keyboard_check_delay(37))
    {
        if (!metaMovedLeft)
        {
            cursorPos = max(1, cursorPos - 1);
    ...
    else if (_keyboard_check_delay(39))
    {
        if (!metaMovedRight)
        {
            if (cursorPos == (string_length(consoleString) + 1) && array_length(filteredSuggestions) != 0)
            {
                suggestion = filteredSuggestions[suggestionIndex];
                consoleWords = _input_string_split(consoleString);
                currentWordLength = string_length(consoleWords[array_length(consoleWords) - 1]);
    ...
    else if (_key_combo_pressed(historyUpModifiers, historyUpKey) && !isAutocompleteOpen)
    {
    ...
    else if (_key_combo_pressed(historyDownModifiers, historyDownKey) && !isAutocompleteOpen)
    {
        if (historyPos < array_length(history))
        {
    ...
    else if (keyboard_check_pressed(vk_enter))
    {
        if (string_trim(consoleString) != "")
        {
            ...
            {
                args = _input_string_split(consoleString);
    ...
    else if (_key_combo_pressed(cycleSuggestionsModifiers, cycleSuggestionsKey) || keyboard_check_pressed(vk_down))
    {
        if (array_length(filteredSuggestions) != 0)
        {
            uncompleted = consoleString;
            consoleString = _find_common_prefix();
            cursorPos = string_length(consoleString) + 1;
            ...
            if (uncompleted == consoleString)
            {
                ...
                if (isAutocompleteOpen)
                    _calculate_scroll_from_suggestion_index();
            }
        }
    }
    else if (_key_combo_pressed(cycleSuggestionsReverseModifiers, cycleSuggestionsReverseKey) || keyboard_check_pressed(vk_up))
    {
        if (array_length(filteredSuggestions) != 0)
        {
    ...
    if (isAutocompleteOpen)
    {
        x1 = autocompleteOriginX;
        y1 = autocompleteOriginY;
        x2 = (x1 + autocompleteMaxWidth + font_get_size(consoleFont) + (autocompletePadding * 2)) - scrollbarWidth;
        y2 = y1 + (string_height(prompt) * min(array_length(filteredSuggestions), autocompleteMaxLines)) + autocompletePadding;
    ...
    lerpValue = (scrollSmoothness == 0) ? 1 : _remap(scrollSmoothness, 1, 0, 0.08, 0.4);
    ...
    if (_shell_properties_hash() != shellPropertiesHash)
...
if (!is_undefined(activeMouseArgType))
{
    ...
    {
        instAtCursor = instance_position(mouse_x, mouse_y, all);

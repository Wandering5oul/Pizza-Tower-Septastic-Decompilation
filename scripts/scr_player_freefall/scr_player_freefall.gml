...
function state_player_freefall()
{
        ...
        if (vsp > 17)
        {
            ...
            else
            {
                ...
                with (create_mach3effect(x, y, sprite_index, image_index, true))
                {
                    ...
                    maxmovespeed = 6;
                    vertical = true;
                    fadeoutstate = UnknownEnum.Value_108;
                }
            }
    ...
    if (grounded && vsp > 0 && (freefallsmash < 10 || !place_meeting(x, y + vsp, obj_metalblock)) && !place_meeting(x, y + 1, obj_destructibles) && !place_meeting(x, y + vsp, obj_destructibles) && !place_meeting(x, y + vsp + 6, obj_destructibles))
    {
        ...
        else
        {
            ...
            if (freefallsmash >= 10)
            {
                with (obj_baddie)
                {
                    if (grounded && point_in_camera(x, y, view_camera[0]) && grounded && vsp > 0 && !invincible && groundpound)
                    {
    ...
    if (chainsawcutscene && (sprite_index == spr_bodyslamstart || sprite_index == spr_shotgunjump1))
        image_speed /= 2;
...
function state_player_freefallsept()
{
    landAnim = 1;
    vsp = freefallvsp;
    freefallvsp += 0.2;

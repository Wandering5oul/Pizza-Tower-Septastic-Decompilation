function scr_string_width(argument0)
{
    ...
    while (pos < string_length_scribble(originalstr))
    {
    ...
    if (array_length(str_arr) == 0)
        w = string_width_scribble(argument0);
    
    for (i = 0; i < array_length(str_arr); i++)
    {
        b = str_arr[i];
        
        if (string_width_scribble(b) > w)
            w = string_width(b);
    }
...
function scr_separate_text(argument0)
{
    ...
    while (scr_string_width(argument0) > (sprite_get_width(spr_dialogue_frame) - (string_width("A") * 5)))
    {
...
{
    if (string_pos(argument1, argument0) != -1)
        return true;
    else
        return false;
}

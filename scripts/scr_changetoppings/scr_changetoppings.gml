function scr_changetoppings()
{
    with (obj_collect)
    {
        if (ds_list_find_index(global.saveroom, id) == -1)
        {
            ...
            repeat (3)
                instance_create(x + (sprite_width / 2) + random_range(-5, 5), y + (sprite_height / 2) + random_range(-5, obj_tubeleft), obj_cloudeffect);
        }
    }
    ...
    with (obj_bigcollect)
    {
        if (ds_list_find_index(global.saveroom, id) == -1)
        {
            ...
            repeat (3)
                instance_create(x + (sprite_width / 2) + random_range(-10, 10), y + (sprite_height / 2) + random_range(-5, obj_tubeleft), obj_cloudeffect);
        }
    }
    ...
    with (obj_giantcollect)
    {
        if (ds_list_find_index(global.saveroom, id) == -1)
        {
            ...
            repeat (5)
                instance_create(x + (sprite_width / 2) + random_range(-15, 15), y + (sprite_height / 2) + random_range(-5, obj_tubeleft), obj_cloudeffect);
        }
    }
